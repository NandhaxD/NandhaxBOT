...
@katsuki.on_message(filters.command("hack",prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def hack(_, m):
...
@katsuki.on_message(filters.command("love",prefixes=config.HANDLER) & filters.user(config.OWNER_ID))
async def love(_, m):
